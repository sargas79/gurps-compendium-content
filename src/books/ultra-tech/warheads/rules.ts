...
import { divideDamage, plusPerDie, stepPiercing, timesRange as times } from "../../../shared/loads/dice.js";
import { loadableOf, type Launcher } from "../../../shared/loads/launcher.js";
...
// The load engine's weapon, row and dice, shared with High-Tech's ammunition (src/shared/loads).
export { divideDamage, plusPerDie, stepPiercing, type Launcher };
...
export function loadable(launcher: Launcher): WarheadKind[] {
  return loadableOf(Object.keys(WARHEADS) as WarheadKind[], (kind) => refusal(kind, launcher));
}

/** A row's figures a warhead changes. */
export type WarheadRow = LoadRow;
